...
exports.formatChatContextForPrompt = formatChatContextForPrompt;
exports.findRelevantFiles = findRelevantFiles;
 ...
 */
async function getActiveFileContext(workspaceRoot) {
 ...
 */
async function getOpenTabsContext(workspaceRoot) {
    const tabs = [];
 ...
 */
async function getRepositoryStructure(workspaceRoot) {
    if (!workspaceRoot) {
        return {
            files: [],
            directories: [],
 ...
 */
function formatChatContextForPrompt(context, maxTokens = 8000) {
    let result = "";
    let tokenCount = 0;
    // Helper to estimate tokens (rough estimate: 4 chars per token)
    const addWithLimit = (text, priority = 1) => {
 ...
 */
async function findRelevantFiles(query, activeFilePath, workspaceRoot) {
    const relevantFiles = [];
    if (!workspaceRoot)
